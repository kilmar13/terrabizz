...
import { X, Sparkles, CloudUpload } from 'lucide-react';
...
export default function VisualNotepadScannerModal({ isOpen, onClose }: Props) {
      ...
      <div className="bg-[#1e2235] rounded-[24px] shadow-2xl animate-in zoom-in-95 duration-200 relative flex flex-col max-h-[90vh] my-auto overflow-hidden w-full max-w-[90vw] md:max-w-lg border border-slate-700 text-white">
        <div className="shrink-0 p-4 sm:p-6 border-b border-slate-700 flex items-center justify-between z-10 w-full sticky top-0 bg-[#1e2235]">
          <div className="flex items-center gap-3">
            <div className="bg-[#292e44] p-2.5 rounded-full text-indigo-400">
              <Sparkles size={20} />
            </div>
            <div>
              <h2 className="text-[13px] font-black tracking-widest uppercase">Visual Notepad Scanner</h2>
              <p className="text-[9px] text-slate-400 uppercase tracking-widest font-bold mt-1">Handwritten & Printed Order Digitizer</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-[#181b2a]">
          <div 
            ...
            className={`border-2 border-dashed rounded-2xl p-8 flex flex-col items-center text-center transition-colors ${
              dragActive ? 'border-indigo-500 bg-indigo-500/10' : 'border-slate-700 hover:border-slate-600 bg-[#1e2235]'
            }`}
          >
            <CloudUpload size={48} className="text-indigo-400 mb-6" />
            <h3 className="text-sm font-black tracking-widest uppercase mb-3">Upload Customer Order Notepad</h3>
            <p className="text-xs text-slate-400 mb-8 leading-relaxed font-bold max-w-sm">
              Snap a photo of the client's handwritten request note, bill list, or message screenshot.
            </p>
