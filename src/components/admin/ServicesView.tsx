...
import { Search, CheckCircle, Ban, Eye, Trash2, Briefcase, AlertCircle } from 'lucide-react';
...
export default function ServicesView() {
  ...
  const handleStatusChange = async (id: string, newStatus: string, title: string) => {
    try {
      ...
      await logAdminAction('UPDATE_SERVICE_STATUS', `Changed service "${title}" status to ${newStatus}`, 'admin');
    } catch (error) {
      console.error(error);
  ...
  const handleDelete = async (id: string, title: string) => {
    if (confirm(`Are you sure you want to delete service "${title}"?`)) {
      try {
        ...
        await logAdminAction('DELETE_SERVICE', `Deleted service "${title}"`, 'admin');
      } catch (error) {
        console.error(error);
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-black tracking-tight text-white uppercase">Service Management</h2>
          <p className="text-slate-400 text-sm mt-1">Review, approve, and manage service listings.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-white/5 border-b border-white/5">
                ...
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Price Model</th>
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                <tr><td colSpan={5} className="px-6 py-8 text-center text-slate-500 font-medium">Loading services...</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={5} className="px-6 py-8 text-center text-slate-500 font-medium">No services found.</td></tr>
              ) : (
                filtered.map((item) => (
                  <tr key={item.id} className="hover:bg-white/[0.02] transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-sm font-bold text-white max-w-[200px] truncate">{item.title || 'Untitled'}</p>
                          <p className="text-[10px] uppercase tracking-widest text-slate-500 mt-1">{item.providerName || 'Unknown Provider'}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <span className="text-sm font-bold text-blue-400">
                        {item.price ? `₦${Number(item.price).toLocaleString()} / ${item.priceUnit || 'hr'}` : 'Contact for quote'}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right space-x-2">
                       ...
                       {item.status !== 'REJECTED' && (
                         <button 
                           onClick={() => handleStatusChange(item.id, 'REJECTED', item.title)}
